...
def setup_logger(log_dir='logs', log_level=logging.INFO):
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
    log_filename = os.path.join(log_dir, 'train_' + f'{time.strftime("%Y%m%d_%H%M%S")}.log')
...
class AllConfig(Config):
    ...
    def parse_args(self):
        ...
        parser.add_argument('--test_time', action='store_true', default=False, help="Set to True for test time inference")
